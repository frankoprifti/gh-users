...
export default function Home({ users: prefetchedUsers }) {
    ...
    const filteredUsers = useMemo(() => {
        if (deferredSearchVal) {
            return users.filter(user => user.login.includes(deferredSearchVal))
        }
        return users
    }, [deferredSearchVal, users])

    const fetchMore = useCallback(async () => {
        const baseUrl = process.env.NEXT_PUBLIC_BASE_URL;
        const nextPage = page + 1;
        const usersReq = await fetch(`${baseUrl}/users?since=${nextPage * 30}&per_page=30`, {
            headers: {
    ...
    return (
        <div id='home-scroll'>
            <div className='mx-4 mt-4 bg-gray-800 px-5 rounded-xl'>
                <input
                    className='w-full h-[44px] outline-0'
            ...
            <div className='grid grid-cols-4 gap-4 p-4'>
                {filteredUsers.map(user => <User key={user.login} user={user} />)}
            </div>
        </div>
    )
}
